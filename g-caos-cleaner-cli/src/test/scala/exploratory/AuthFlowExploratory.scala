...
import org.apache.hc.core5.http.impl.io.{DefaultClassicHttpResponseFactory, DefaultHttpRequestParser, DefaultHttpResponseWriter, SessionInputBufferImpl, SessionOutputBufferImpl}
...
class AuthFlowExploratory extends FunSuite {
  ...
  test("Apache read request") {
    val server = ServerSocket(9999)
    val clientSocket = server.accept

    val requestParser = DefaultHttpRequestParser()
    val sessionInputBuffer = SessionInputBufferImpl(512)
    val parsedRequest = requestParser.parse(sessionInputBuffer, clientSocket.getInputStream)
    val queryParams = URLEncodedUtils.parse(parsedRequest.getUri, StandardCharsets.UTF_8)
    val queryMap = mutable.Map.empty[String, String]

    queryParams.forEach(queryParam => queryMap += (queryParam.getName -> queryParam.getValue))
  ...
  test("Auth flow - Request access token") {
    val client = SimpleHttpClient()
    val request = basicRequest.post(uri"http://localhost:5000/access-token")
      .body(AuthAccessTokenRequest("4/0AWgavdcVShLIgShUCn6uIjUWuUBgLv9lBvsK3CxxwAwaspG4amANyVZbqRzej_qPgedUCA"))
  ...
  test("Call google api") {
    ...
    val credential = Credential.Builder(BearerToken.authorizationHeaderAccessMethod())
      ...
      .build()
    credential.setAccessToken("ya29.a0AX9GBdXqoF-NpnF10CqVXqW3EQLsP_yl_m8kUb6MV6lc9ox5xrwseVkNZth32gAT2UK0qE_I6rLdstIxC9upR3F0xp0zWZcZjoqWC7KX0otlM3hv0yqrqIUoWg0M-qhGUKxIociy4elhtn5DB2sfvhzZ93hlaCgYKAdoSARMSFQHUCsbCMmFjs5FqK7ykvRk94KR6zw0163")
    val gmail = Gmail.Builder(transport, GsonFactory.getDefaultInstance, credential).setApplicationName("g-caos-cleaner").build()
    val labelsResponse = gmail.users.labels.list("me").execute
    labelsResponse.getLabels.stream().map(label => label.getName).forEach(println)
  }

  case class AuthAccessTokenRequest(authorization_code: String)
  implicit val myRequestRW: ReadWriter[AuthAccessTokenRequest] = macroRW[AuthAccessTokenRequest]
  ...
  def httpResponse(): BasicClassicHttpResponse = BasicClassicHttpResponse(200, "OK")
